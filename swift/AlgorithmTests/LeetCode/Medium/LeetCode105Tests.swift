...
struct LeetCode105Tests {
  ...
  func example1() async {
    ...
    #expect(result == [3, 9, 20, nil, nil, 15, 7], #"Expected '[3, 9, 20, nil, nil, 15, 7]', but got '\#(result)'"#)
  }
  ...
  func example3() {
    ...
    #expect(result == [1, 2, 3, 4, 5, 6, 7], #"Expected '[1,2,3,4,5,6,7]', but got '\#(result)'"#)
  }

  @Test(.timeLimit(.minutes(1)))
  func example4() {
    let result = problem.buildTree([1, 2, 3, 4, 5, 6], [4, 3, 5, 2, 6, 1])
    #expect(result == [1, 2, nil, 3, 6, 4, 5], #"Expected '[1,2,nil,3,6,4,5]', but got '\#(result)'"#)
  }
}
