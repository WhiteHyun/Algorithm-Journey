...
struct LeetCode3217Tests {
  ...
  func example1() throws {
    let result = try #require(problem.modifiedList([1, 2, 3], [1, 2, 3, 4, 5]))
    #expect(result == [4, 5], #"Expected '[4, 5]', but got '\#(result)'"#)
  }

  @Test
  func example2() throws {
    let result = try #require(problem.modifiedList([1], [1, 2, 1, 2, 1, 2]))
    #expect(result == [2, 2, 2], #"Expected '[2, 2, 2]', but got '\#(result)'"#)
  }

  @Test
  func example3() throws {
    let result = try #require(problem.modifiedList([5], [1, 2, 3, 4]))
